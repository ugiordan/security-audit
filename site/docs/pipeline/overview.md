...
Creates the output directory structure under `output/<repo-name>/<timestamp>/` and initializes a session log (`session-log.json`) that tracks timing and status for each pipeline stage.
...
Updates `output/security-trends.json` with the scan results for historical tracking. Finalizes the session log with total duration and exit status.
...
python3 pipeline.py opendatahub-io/kube-auth-proxy --reports-only --scan-dir output/kube-auth-proxy/2026-06-04-142201
```
